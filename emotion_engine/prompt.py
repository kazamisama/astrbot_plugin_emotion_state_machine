...
# v0.9.55: Human-readable label translations. The internal label keys
# (English) stay stable across versions and storage; only the display
# text is localized. To add a language, copy this dict with translated
# values and swap at the format_* boundary.
...
def _display_label(label: str) -> str:
    """Translate an internal label to its display string (zh-CN)."""
...
def format_snapshot(scope: str, snapshot: GroupEmotionSnapshot) -> str:
    ...
    return (
        ...
        f"- \u6807\u7b7e: {_display_label(snapshot.label)}\n"
...
def format_group_chart(scope: str, snapshot: GroupEmotionSnapshot) -> str:
    ...
    lines = [
        ...
        f"  \u6109\u60a6\u5ea6     {_bar(snapshot.valence)} {snapshot.valence:.1f}  {_display_label(snapshot.label)}",
...
def format_relation_chart(scope: str, user_id: str, snapshot: UserRelationSnapshot) -> str:
    ...
    lines = [
        ...
        f"  \u4fe1\u4efb         {_bar(snapshot.trust)} {snapshot.trust:.1f}  {_display_label(snapshot.label)}",
...
def _build_emotion_block_variables(
    ...
    if relation is not None:
        ...
        relation_block = (
            ...
            f"\u5bf9\u5f53\u524d\u7528\u6237: \u5173\u7cfb\u6807\u7b7e={_display_label(relation.label)}, "
